...
def get_LS(time, flux,freq):
    ...
    plt.text(freq[0], FP_90, '95%',font1)
...
def read_SAS_lc():
    ...
    os.chdir(path)
    mode=['mos1','mos2','pn']
    filename1=mode[0]+'_lccorr_bin{0}.lc'.format(int(dt));filename2=mode[1]+'_lccorr_bin{0}.lc'.format(int(dt))
    filename3=mode[2]+'_lccorr_bin{0}.lc'.format(int(dt))
    # filename3=mode[2]+'_src_lc_bin{0}.lc'.format(int(dt))
    ...
    freq=np.arange(1./50000,0.5/dt,1./(10*50000))
