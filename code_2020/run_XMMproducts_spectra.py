...
for obsID in obsList:
   ...
   if process:
      if mypath.is_dir():
         ...
         v1=os.popen("ls *M1*S*EVLI*.FIT")
         mos1name=v1.read()[0:-1]
         v2=os.popen("ls *M2*S*EVLI*.FIT")
   ...
   # ---------------------------------------------
   srcName = "vcc944"
   srcReg = "circle(33926.994,41531.187,2380.6631)"
   bkgReg = "box(32043.414,35291.846,2314.8148,3271.6049,0)"

   ## 改成你所选取的region ##

   if filt_particle_bkg:
      pn_threshold=1000.5;mos_threshold=1000
      for det in detList:
      ...
      for det in detList:
         ...
         if det == "pn":
             cmd = "evselect table="+det+"_filt_time_bary.fits energycolumn=PI " \
                                                  "expression='#XMMEA_EP && (PATTERN<=4) && ((X,Y) IN "+srcReg+ ")"+" &&(PI in [2000:10000])' withrateset=yes rateset="\
                   +det+"_src_lc_bin{0}.lc timebinsize={0} maketimecolumn=yes makeratecolumn=yes timemin={1} timemax={2}".format(lenbin,tmin,tmax)
         else:
            cmd = "evselect table=" + det + "_filt_time_bary.fits energycolumn=PI " \
                                                       "expression='#XMMEA_EM && (PATTERN<=12) && ((X,Y) IN " + srcReg + ")"+" &&(PI in [2000:10000])' withrateset=yes rateset=" \
                  + det + "_src_lc_bin{0}.lc timebinsize={0} maketimecolumn=yes makeratecolumn=yes timemin={1} timemax={2}".format(lenbin, tmin, tmax)
         print(" ")
         ...
         if det == "pn":
             cmd = "evselect table="+det+"_filt_time_bary.fits energycolumn=PI " \
                                                  "expression='#XMMEA_EP && (PATTERN<=4) && ((X,Y) IN "+bkgReg+")"+" &&(PI in [2000:10000])' withrateset=yes rateset="\
                   +det+"_bkg_lc_bin{0}.lc timebinsize={0} maketimecolumn=yes makeratecolumn=yes timemin={1} timemax={2}".format(lenbin,tmin,tmax)
         else:
            cmd = "evselect table=" + det + "_filt_time_bary.fits energycolumn=PI " \
                                                       "expression='#XMMEA_EM && (PATTERN<=12) && ((X,Y) IN " + bkgReg + ")"+" &&(PI in [2000:10000])' withrateset=yes rateset=" \
                  + det + "_bkg_lc_bin{0}.lc timebinsize={0} maketimecolumn=yes makeratecolumn=yes timemin={1} timemax={2}".format(lenbin, tmin, tmax)
         print(" ")
